...
export function renderAll() {
  ...
  gameState.zones.hand.forEach((cardId) => {
    elements.handZone.appendChild(
      createCardElement(cardId, false, "hand", "drag")
    );
  });
...
export function createCardElement(
  ...
  if (zoneId === "hand" || zoneId === "stage" || zoneId === "direction") {
    cardEl.addEventListener("click", (e) =>
      handleTap(cardEl, { zoneId, slotIndex, slotColor, cardId: actualCardId })
    );
  }
  return cardEl;
}
...
export function handlePressStart(e, element, zoneId, slotIndex, slotColor) {
  ...
  longPressTimer = setTimeout(() => {
    if (!isDragging && element.classList.contains("card") && !isPile) {
      ...
      if (
        cardIdToZoom &&
        elements.cardZoomOverlay &&
        elements.zoomedCardImage
      ) {
...
export function changeBackground() {
  ...
  if (!currentBg || currentBg === "" || currentBg === "none") {
    ...
    currentBg.includes("wall.webp") &&
    !currentBg.includes("wall1.webp") &&
    !currentBg.includes("wall2.webp")
  ) {
                                                    ...
                                                    : ""
                                                     ...
                                                     style="z-index: ${
                                                       cardIndex + 1
                                                            ...
                                                            background-image: url('${CARD_IMAGE_PATH}${
                                                  card.cardId
                                                }.webp');
                                                            background-size: 180%;
...
export function setupEventListeners() {
  ...
  elements.moveHandToTrashBtn.addEventListener("click", () => {
    moveHandToTrash();
    renderAll();
  });
  elements.sortHandBtn.addEventListener("click", () => {
    sortHand();
    renderAll();
  });

  elements.trashZone.addEventListener("mousedown", (e) =>
    handlePressStart(e, elements.trashZone, "trash")
  );
  elements.trashZone.addEventListener(
    "touchstart",
    (e) => handlePressStart(e, elements.trashZone, "trash"),
    { passive: false }
  );

  elements.deckZone.addEventListener("mousedown", (e) =>
    handlePressStart(e, elements.deckZone, "deck")
  );
  elements.deckZone.addEventListener(
    "touchstart",
    (e) => handlePressStart(e, elements.deckZone, "deck"),
    { passive: false }
  );

  elements.volNoiseZone.addEventListener("mousedown", (e) =>
    handlePressStart(e, elements.volNoiseZone, "volNoise")
  );
  elements.volNoiseZone.addEventListener(
    "touchstart",
    (e) => handlePressStart(e, elements.volNoiseZone, "volNoise"),
    { passive: false }
  );
  ...
  document.addEventListener("click", (event) => {
    ...
    if (elements.temporaryExpandedZone.style.display === "flex") {
      if (
        elements.openTemporaryZoneBtn.contains(event.target) ||
        elements.temporaryExpandedZone.contains(event.target)
      ) {
        return;
      }
      elements.temporaryExpandedZone.style.display = "none";
  ...
  elements.clearDeckP1Btn.addEventListener("click", () => {
    clearDeckData("p1");
  });
  elements.clearDeckP2Btn.addEventListener("click", () => {
    clearDeckData("p2");
  });
    ...
    if (isDualMode) {
      ...
      if (deckString1.startsWith("KCG-")) {
        ...
        if (deckList1.length === 0) {
          alert(
            "プレイヤー1のKCGコードの解析に失敗しました。正しいコードを入力してください。"
          );
          return;
        }
      } else {
      ...
      if (deckString2.startsWith("KCG-")) {
        ...
        if (deckList2.length === 0) {
          alert(
            "プレイヤー2のKCGコードの解析に失敗しました。正しいコードを入力してください。"
          );
          return;
        }
      } else {
      ...
      if (deckString.startsWith("KCG-")) {
        ...
        if (deckList.length === 0) {
          alert(
            "KCGコードの解析に失敗しました。正しいコードを入力してください。"
          );
          return;
        }
      } else {
